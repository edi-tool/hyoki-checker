...
- **レスポンシブ化**: 1023px 以下で右パネルを下に回して縦積み（従来は 390px 幅で横にはみ出し、入力欄が縦書き状に潰れていた）。
- **PC の作業領域**: `h-full` が効かず入力欄が数行に縮んでいたため、左右の列を stretch させ、作業領域を画面の高さ（`100dvh - 150px`）に収めた。入力・プレビュー・結果はそれぞれ枠内でスクロール。
- 見出しを他ツールと揃えた（h1 18px/600、サブ 13px、上に「edi-tool」リンク）。フッターも 12px・区切り線なしに統一し、ツール一覧へのリンクを追加。
